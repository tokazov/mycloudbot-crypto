...
import hashlib, json, base64, httpx, os
...
async def create_payment(uid: int):
    ...
    payload = {
        ...
        "currency": "USD",
        "order_id": order_id,
        "url_callback": "https://crypto-service.up.railway.app/webhook",
...
async def webhook(request: Request):
    ...
    if data.get("payment_status") in ("paid", "paid_over"):
        ...
        if len(parts) >= 2 and parts[0] == "mcb":
            ...
            # Уведомляем бот через Telegram API
            async with httpx.AsyncClient(timeout=10) as client:
                await client.post(
                    f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage",
                    json={"chat_id": uid, "text": "✅ Оплата подтверждена! Подписка активна на 30 дней. Твой AI-партнёр работает 🚀", "parse_mode": "HTML"}
                )
                await client.post(
                    f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage",
                    json={"chat_id": ADMIN_ID, "text": f"💰 Крипто-оплата!\n🆔 {uid}\n💵 ${PRICE_USD} USDT\n📋 {order_id}", "parse_mode": "HTML"}
                )
            # Активируем подписку через внутренний API бота
            async with httpx.AsyncClient(timeout=5) as client:
                try:
                    await client.post(
                        "http://46.225.80.44:8080/internal/activate",
